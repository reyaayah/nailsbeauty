...
import { ShoppingBag, ChevronLeft, Check, Loader2, MapPin, CreditCard, Package } from "lucide-react";
...
export default function CheckoutPage() {
    ...
    async function handlePlaceOrder() {
        ...
        setPlacing(true);
        try {
            const id = await placeOrder(user.uid, items, subtotal, address, paymentMethod);
            setOrderId(id);
            await clearCart();
            setStep("success");
        } catch (err) {
                                ...
                                <div key={s.key} className="flex items-center">
                                    ...
                                    >
                                        ...
                                        <span
                                            ...
                                            style={{
                                                color: i <= stepIndex ? theme.colors.dark : theme.colors.muted,
                                            }}
                                        >
                                            {s.label}
                                        </span>
                                    ...
                                    {i < STEPS.length - 1 && (
                                        <div
                                            ...
                                            style={{
                                                backgroundColor:
                                                    i < stepIndex ? theme.colors.primary : `${theme.colors.dark}20`,
                                            }}
                                        />
                                    )}
                                </div>
                                            ...
                                            <div key={`${product.id}-${size}-${shape}`} className="flex gap-3 items-center">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <p className="text-[10px] opacity-50 mt-0.5" style={{ color: theme.colors.dark }}>
                                                        {[size && `Size: ${size}`, shape && `Shape: ${shape}`, `Qty: ${quantity}`]
                                                            .filter(Boolean)
                                                            .join(" · ")}
                                                    </p>
                                                </div>
                            ...
                            <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
                                {items.map(({ product, quantity, size, shape }) => (
                                    <div key={`${product.id}-${size}-${shape}`} className="flex gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="text-xs font-medium truncate" style={{ color: theme.colors.dark }}>
                                                {product.name}
                                            </p>
    ...
    return (
        <div className={`flex flex-col gap-1 ${colSpan}`}>
            ...
            <input
                ...
                style={{
                    borderColor: error ? "#ef4444" : `${theme.colors.dark}20`,
                    color: theme.colors.dark,
                    backgroundColor: theme.colors.light,
                }}
            />
